...
\begin{figure}
	...
	\caption{(a) Acquisition with blended crossline ($x$ direction) sources: The blending process is described at each inline position with a blending matrix which has as many rows as crossline sources, $Ns_x$. Such a blending matrix exists for each of the $Ns_y$ inline positions. (b) All these blending matrices are assembled in a single blending matrix, $\mathbf{\Gamma}$, which has $Ns_x$ by $Ns_y$ rows.}
