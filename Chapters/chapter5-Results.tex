...
In section \ref{sec:BlendingMatrix} it was shown that for an incoherent blending pattern the elements, $\mathrm{e}^{-j \omega \Delta t_{kl}}$, along a sub-diagonal of the product $\mathbf{\Gamma \Gamma}^H$ should be out of phase. The phase variation of the sub-diagonal elements will be used to quantify incoherency.

The sub-diagonal elements, $\mathrm{e}^{-j \omega \Delta t_{kl}}$, are complex numbers with unit length and variable phase. Hence, they are located in the complex plane on a circle with radius 1 (see Figure \ref{fig:Ch-Results-complex-circle}). 
...
The elements along the $k^{th}$ sub-diagonal are summed. The length of the resulting complex number indicates if the elements superimposed constructively or destructively. The resulting number is squared in order to put it in terms of energy;
