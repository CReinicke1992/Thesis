...
The sub-diagonal elements, $\mathrm{e}^{-j \omega \Delta t_{kl}}$, are complex numbers with length 1 and variable phase. Hence, these elements are located on a unit circle in the complex number plane (see Figure \ref{fig:Ch-Results-complex-circle}). 
...
If all elements are in phase the absolute value of their sum is maximized. The more the elements are out of phase, i.e. the more incoherent they are, the smaller the length of the summed elements. Figure \ref{fig:Ch-Results-Diagonal-Sums} illustrates the length of the summed sub-diagonals. For a perfectly incoherent blending design the elements on a sub-diagonal cancel and the plot becomes a perfect spike.


\begin{figure}
	\centering
	\includegraphics{Plots/diagonal-sums}
	\caption{The sub-diagonal elements, $\mathrm{e}^{-j \omega \Delta t_{kl}}$, of the product $\mathbf{\Gamma \Gamma}^H$ are summed. The length of each output is plotted as a function of the diagonal index. On the main diagonal all elements of the product $\mathbf{\Gamma \Gamma}^H$ are in phase, which results in a spike.}
	\label{fig:Ch-Results-Diagonal-Sums}
\end{figure}

In section \ref{sec:BlendingMatrix} it was also demonstrated that the main diagonal of the product $\mathbf{\Gamma \Gamma}^H$ only contains ones. Consequently, the sum along the main diagonal 
