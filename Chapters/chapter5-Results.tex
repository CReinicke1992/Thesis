...
The incoherency, $\mu$, is quantified as follows: The elements are summed along sub-diagonals of $\mathbf{\Gamma \Gamma}^H$. Each sum is taken in absolute terms and squared. This gives one value per sub-diagonal and per frequency component, $\omega$. These values are summed over all sub-diagonals and over all frequencies, $\omega$. The output value measures the total coherent energy in the matrix $\mathbf{\Gamma \Gamma}^H$. The same calculation is performed for the main diagonal only. 

 The incoherency is the ratio of the coherent energy on the main diagonal of $\mathbf{\Gamma \Gamma}^H$ and the energy of the total coherent energy of $\mathbf{\Gamma \Gamma}^H$;
