...
The incoherency, $\mu$, is quantified as follows: The elements are summed along sub-diagonals of $\mathbf{\Gamma \Gamma}^H$. Each sum is taken in absolute terms and squared. This gives one value per sub-diagonal and per frequency component, $\omega$. First these values are summed. Then, the computation is repeated for each frequency component, $\omega$, and all frequency components are summed. The same calculation is performed for the main diagonal only. The incoherency is the ratio of the two values;
...
Note that $N_s$ is the number of sources.
