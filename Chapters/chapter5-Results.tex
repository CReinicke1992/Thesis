...
The sub-diagonal elements, $\mathrm{e}^{-j \omega \Delta t_{kl}}$, are complex numbers with length 1 and variable phase (see Figure \ref{fig:Ch-Results-complex-circle}). Hence, these elements are located on a unit circle in the complex number plane. If all elements are in phase the absolute value of their sum is maximized. However, if the elements have random phases the absolute value of their sum is small.
