...
The incoherency, $\mu$, is quantified as follows: The length of the summed sub-diagonals of $\mathbf{\Gamma \Gamma}^H$ is squared. This gives one value per sub-diagonal. First these values are summed. Then, the computation is repeated for each frequency component, $\omega$, and all frequency components are summed. The same calculation is performed for the main diagonal only. The incoherency is the ratio of the two values;
