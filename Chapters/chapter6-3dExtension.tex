...
In section \ref{sec:IterBlenNoiseEst} the 2D $f$-$k$ filter was introduced. By sorting the 2D $f$-$k$-fitler according to section \ref{sec:Ch-Theory-3dExtension-DataSorting} it can be applied to 3D data (see Figure \ref{fig:Ch-Mahdad3d-2dfk}). However, it only filters the crossline wavenumbers $k_x$. Thus, it is a $f$-$k_x$-filter.

By taking into account both spatial direction, $x$ and $y$, the filter can be extended to a 3D $f$-$k_x$-$k_y$-filter.
